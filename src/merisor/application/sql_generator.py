...
class MLDSQLValidator:
    ...
    def validate(
        self, model: MLDModel, target: SQLTarget | str
    ) -> SQLValidationReport:
        ...
        for table in model.tables:
            ...
            for index in table.indexes:
                ...
                if self._is_reserved(index.name, dialect):
                    issues.append(
                        self._reserved_warning("index", index.name, table.id)
                    )
        return SQLValidationReport(tuple(issues))
    ...
    def _validate_auto_increment(
        ...
        if not column.auto_increment:
            return
        if not isinstance(column.data_type, MLDDataType) or column.data_type.name not in {
    ...
    def _error(
        ...
        return SQLValidationIssue(
            SQLValidationSeverity.ERROR, code, message, table_id
        )
...
class SQLGenerator:
    ...
    def generate(
        ...
        *,
        ...
        deferred_keys = (
            cyclic_keys if dialect.supports_alter_add_foreign_key else set()
        )
                ...
                if (table.id, foreign_key.id) not in deferred_keys:
                    definitions.append(
                        self._foreign_key_clause(
                            model, table, foreign_key, dialect
                        )
                    )
    ...
    def _alter_foreign_key_sql(
        ...
        return (
            f"ALTER TABLE {dialect.quote_identifier(table.name)}\n"
            f"    ADD {clause};"
        )

    def _index_sql(
        self, table: MLDTable, index: MLDIndex, dialect: SQLDialect
    ) -> str:
    ...
    def _cyclic_foreign_keys(self, model: MLDModel) -> set[tuple[str, str]]:
        adjacency = {table.id: set() for table in model.tables}
        for table in model.tables:
        ...
        for table in model.tables:
            for foreign_key in table.foreign_keys:
                ...
                if (
                    len(component) > 1
                    and foreign_key.referenced_table_id in component
                ):
    ...
    def _ordered_tables(
        ...
        dependencies = {table.id: set() for table in model.tables}
        for table in model.tables:
    ...
    def _validate_generated_script(script: str, *, expected_tables: int) -> None:
        ...
        if depth:
            raise RuntimeError("Le SQL généré contient des parenthèses non équilibrées.")
        if expected_tables and script.count("CREATE TABLE ") != expected_tables:
            raise RuntimeError("Le SQL généré ne contient pas toutes les tables attendues.")
        if ";;" in script:
